...
use {
    crate::{
        ...
        state::{
            collect_staged_features_signer_seeds, collect_validator_support_signal_signer_seeds,
            get_staged_features_address, get_staged_features_address_and_bump_seed,
            get_validator_support_signal_address,
            get_validator_support_signal_address_and_bump_seed, FeatureBitMask, StagedFeatures,
...
fn process_stage_feature_for_activation(
    ...
    // Check to see if the staged features account should be initialized.
    if staged_features_info.data_is_empty() {
        let _system_program_info = next_account_info(account_info_iter)?;

        let (staged_features_address, bump_seed) =
            get_staged_features_address_and_bump_seed(&upcoming_epoch);
        let bump_seed = [bump_seed];
        let epoch_as_le = upcoming_epoch.to_le_bytes();
        let signer_seeds = collect_staged_features_signer_seeds(&epoch_as_le, &bump_seed);

        // Ensure the staged features address is the correct address derived
        // from the upcoming epoch.
        if !staged_features_info.key.eq(&staged_features_address) {
        ...
        invoke_signed(
            &system_instruction::allocate(
                ...
                std::mem::size_of::<StagedFeatures>() as u64,
            ),
            &[staged_features_info.clone()],
            &[&signer_seeds],
        )?;
        invoke_signed(
            &system_instruction::assign(&staged_features_address, program_id),
            &[staged_features_info.clone()],
            &[&signer_seeds],
        )?;
    } else {
...
fn process_signal_support_for_staged_features(
    ...
    if vote_account_epoch_stake != 0 {
        // Check to see if the validator support signal account should be
        // initialized.
        if validator_support_signal_info.data_is_empty() {
            let _system_program_info = next_account_info(account_info_iter)?;

            let (validator_support_signal_address, bump_seed) =
                get_validator_support_signal_address_and_bump_seed(vote_account_info.key);
            let bump_seed = [bump_seed];
            let signer_seeds =
                collect_validator_support_signal_signer_seeds(vote_account_info.key, &bump_seed);
            ...
            if !validator_support_signal_info
                .key
                .eq(&validator_support_signal_address)
            {
            ...
            invoke_signed(
                &system_instruction::allocate(
                    validator_support_signal_info.key,
                    std::mem::size_of::<ValidatorSupportSignal>() as u64,
                ),
                &[validator_support_signal_info.clone()],
                &[&signer_seeds],
            )?;
            invoke_signed(
                &system_instruction::assign(validator_support_signal_info.key, program_id),
                &[validator_support_signal_info.clone()],
                &[&signer_seeds],
            )?;

            // Add the validator's stake in support of the signaled features.
            staged_features.add_stake_support(&signal, vote_account_epoch_stake);

            // Write the signal to the validator support signal account.
            ...
            unpack_pod_mut::<ValidatorSupportSignal>(&mut validator_support_signal_data)
                .map(|s| s.store_signal(current_epoch, signal))?;
        } else {
            // Ensure the validator support signal address is the correct address
            // derived from the vote account and the _current_ epoch.
            if !validator_support_signal_info
